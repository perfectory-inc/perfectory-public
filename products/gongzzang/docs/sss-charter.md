...
> 적용: perfectory 모노레포의 제품·플랫폼 영역과 이들이 발행하는 서비스 계약
> ([ADR 0045](./adr/0045-adr-placement-cross-repo-governance.md)).

---

## 0. 최상위 규칙 — 제품 우선 (SSS보다 우선)

런칭 전 단계에서 SSS는 **"그렇게 *될 수 있게* 설계하라"는 방향**이지,
...
*최소선:* format/lint, gitleaks, dependency policy, artifact-size 정책, 생성물 drift guard.
*수요 기반 확장:* OpenAPI diff, coverage gate, image signing/SBOM.
...
- **런칭 전 최소선:** Catalog 원본=Foundation Platform lineage store, 자체 외부호출=승인된 archive/lineage 계약
...
- **런칭 전 최소선:** gitleaks·cargo-deny 그린(수용 취약점은 deny.toml에 사유+TODO). 권한정책=registry→생성코드 단일출처.
  PII 로그 0. 공개된 취약점은 정책에 따라 차단하거나 사유·기한·소유자를 기록한다.
