...
for t in "${FORBIDDEN_TABLES[@]}"; do
  if psql "$DATABASE_URL" -t -A -c "select 1 from pg_tables where schemaname='public' and tablename='$t';" | grep -q '^1$'; then
    echo "FAIL: Foundation-owned table must not exist in the Gongzzang database: $t" >&2
    exit 1
  fi
done
...
FPEI_PAYLOAD_CHECK=$(psql "$DATABASE_URL" -t -A -c "select 1 from pg_constraint where conrelid='foundation_platform_event_inbox'::regclass and conname='foundation_platform_event_inbox_anchor_payload_chk';")
if [ "$FPEI_PAYLOAD_CHECK" != "1" ]; then
  echo "FAIL: foundation_platform_event_inbox_anchor_payload_chk missing" >&2
  exit 1
fi

FPEI_PENDING_IDX=$(psql "$DATABASE_URL" -t -A -c "select count(*) from pg_indexes where schemaname='public' and tablename='foundation_platform_event_inbox' and indexname='foundation_platform_event_inbox_pending_idx';")
if [ "$FPEI_PENDING_IDX" != "1" ]; then
  echo "FAIL: foundation_platform_event_inbox_pending_idx missing" >&2
  exit 1
fi

FPEI_ANCHOR_SNAPSHOT_IDX=$(psql "$DATABASE_URL" -t -A -c "select count(*) from pg_indexes where schemaname='public' and tablename='foundation_platform_event_inbox' and indexname='foundation_platform_event_inbox_anchor_snapshot_idx';")
if [ "$FPEI_ANCHOR_SNAPSHOT_IDX" != "1" ]; then
  echo "FAIL: foundation_platform_event_inbox_anchor_snapshot_idx missing" >&2
  exit 1
fi
