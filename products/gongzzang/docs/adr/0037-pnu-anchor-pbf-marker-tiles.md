...
- marker location is resolved from a foundation-platform PNU anchor, not from arbitrary listing
  coordinates.
...
Naver Marker objects; it must use the foundation-platform PNU-anchor PBF marker contract.

Known transitional areas:

- `apps/web/components/listings/listing-map.tsx` now registers the foundation-platform `parcel_anchor`
...
- `crates/listing-domain/src/repository.rs` and `crates/gongzzang-persistence/src/listing.rs` no longer expose
...
- `services/gongzzang-api/src/routes/listings.rs` no longer accepts public `bounds` query input.
- `services/gongzzang-api/src/routes/listing_marker_tiles.rs` exposes the Gongzzang listing PBF endpoint:
  `GET /map/v1/marker-tiles/listing/{z}/{x}/{y}.pbf?filter_hash=all-active-v1`.
- `crates/gongzzang-persistence/src/listing.rs` exposes `find_card_summaries`, not a bbox-named card query.
...
- render the marker at the foundation-platform anchor for that PNU;
...
- treating the PBF tile as the source of truth for the anchor instead of a projection of the
  foundation-platform anchor registry.
...
4. Add frontend PBF source/layer loading and canvas/vector rendering probes. **Done for the
   foundation-platform `parcel_anchor` marker layer and the Gongzzang `listing` marker layer.**
...
- foundation-platform must provide the anchor registry and public/reference marker tile contracts;
...
- foundation-platform anchor generation changes algorithm or geometry source;
