...
/// production 과 동일한 [`product_identity_infrastructure::middleware::AuthenticatedUser`] 주입.
async fn inject_synthetic_auth(mut req: Request, next: Next) -> Response {
    ...
    .expect("test user");
    let claims = product_identity_infrastructure::claims::Claims {
        ...
        nbf: None,
        iss: "test".to_owned(),
        aud: product_identity_infrastructure::claims::Audience::Single("test".to_owned()),
    };
    req.extensions_mut()
...
async fn get_parcel_returns_404_for_unknown_pnu() {
    ...
    let unknown_pnu = "9999900101100000000";
    let url = format!("{base}/api/parcels/{unknown_pnu}");
...
#[tokio::test]
async fn get_parcel_returns_400_for_hub_register_dialect_pnu() {
    let base = spawn_test_app().await;
    let hub_dialect = "9999900101000010000";
    let url = format!("{base}/api/parcels/{hub_dialect}");
...
async fn list_buildings_returns_empty_with_noop_reader() {
    let base = spawn_test_app().await;
    let pnu = "9999900101100010000"; // valid 19-digit repository-reserved fixture
