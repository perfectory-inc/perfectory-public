...
use product_identity_infrastructure::jwks_cache::JwksCache;
use product_identity_infrastructure::verifier::{JwtVerifier, Verifier};
...
/// Production requires Foundation Platform Catalog because canonical building data is
...
fn build_foundation_service_auth(
    workload_identity_token_file: Option<String>,
) -> Result<FoundationServiceAuth, StartupError> {
    let token_file = workload_identity_token_file.ok_or_else(|| {
        production_config_error(
            "FOUNDATION_PLATFORM_WORKLOAD_IDENTITY_TOKEN_FILE must be set because Foundation Platform APIs require a Zitadel workload bearer",
        )
    })?;
    FoundationServiceAuth::from_workload_identity_token_file(token_file.as_str())
...
fn build_building_reader_from_foundation_platform_base_url(
    is_production: bool,
    base_url: Option<String>,
    workload_identity_token_file: Option<String>,
) -> Result<Arc<dyn routes::buildings::BuildingRegisterReader>, StartupError> {
    build_building_reader_from_foundation_config(
        is_production,
        base_url,
        workload_identity_token_file,
    )
...
fn build_building_reader_from_foundation_config(
    is_production: bool,
    base_url: Option<String>,
    workload_identity_token_file: Option<String>,
) -> Result<Arc<dyn routes::buildings::BuildingRegisterReader>, StartupError> {
    if let Some(base_url) = base_url {
        let auth = build_foundation_service_auth(workload_identity_token_file)?;
        tracing::info!("building_register: Foundation Platform Catalog live");
        return building_reader::build_foundation_platform_building_register_reader(
            &base_url,
            Some(auth),
        )
        .map_err(|error| {
            production_config_error(format!(
                "FOUNDATION_PLATFORM_API_BASE_URL invalid for building_register: {error}"
            ))
        });
    }
    if is_production {
        return Err(production_config_error(
            "FOUNDATION_PLATFORM_API_BASE_URL must be set for building_register because Foundation Platform owns catalog building data",
        ));
    }
    tracing::warn!(
