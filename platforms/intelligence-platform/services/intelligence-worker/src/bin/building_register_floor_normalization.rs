...
use intelligence_worker::floor_proposal_job::{
    building_register_floor_proposal_dry_run_enabled_from_env, proposal_generator_from_env,
...
async fn main() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let config = BuildingRegisterFloorProposalJobConfig::from_env()
        .map_err(|message| io::Error::new(io::ErrorKind::InvalidInput, message))?;
    let dry_run = building_register_floor_proposal_dry_run_enabled_from_env()
        .map_err(|message| io::Error::new(io::ErrorKind::InvalidInput, message))?;
    ...
    if dry_run {
        let summary = run_building_register_floor_proposal_dry_run(config, generator).await?;
    ...
    let summary = run_building_register_floor_proposal_job(config, generator, submitter).await?;
    println!("{}", serde_json::to_string_pretty(&summary)?);
