//! Read-only `CourtAuction` aggregate.
...
/// Storage-agnostic, read-only `CourtAuction` aggregate.
///
/// Represents one observed court-auction case. A parcel (`Pnu`) may have multiple cases.
///
...
pub struct CourtAuction {
    ...
    /// Source observation timestamp supplied by the reader implementation.
...
mod tests {
    ...
    fn court_auction_constructs_full_happy_path() {
        let ca = CourtAuction {
            ...
            pnu: Pnu::try_new("9999900101100010000").unwrap(),
            ...
            sold_price: None,
            sold_at: None,
            geom: Some(PointSrid::try_new_wgs84(127.123_470_234_54, 36.123_456_6).unwrap()),
            fetched_at: Utc::now(),
        };
    ...
    fn court_auction_serde_roundtrip() {
        let ca = CourtAuction {
            ...
            sold_price: None,
            sold_at: None,
            geom: Some(PointSrid::try_new_wgs84(127.123_460, 36.123_450).unwrap()),
            fetched_at: Utc::now(),
        };
