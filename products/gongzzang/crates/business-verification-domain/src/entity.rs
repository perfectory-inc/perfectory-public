//! `BusinessVerification` Aggregate (4-status workflow + version OCC + 24h SLA).
//!
...
use crate::errors::BusinessVerificationError;
...
///
/// `submitted_documents` 는 R2 객체 키들의 JSON 배열 형태 (예: `["business_verification/abc/file1.pdf", ...]`).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct BusinessVerification {
    /// 식별자 (`business_verification_<26 ULID>`).
    pub id: Id<BusinessVerificationMarker>,
    /// 제출 사용자 (FK → `user.id`).
...
impl BusinessVerification {
    ...
    ///
    /// `BusinessVerificationStatus::can_transition_to` 로 도메인 머신 검사.
    const fn transition_to(
        &mut self,
        target: BusinessVerificationStatus,
        at: DateTime<Utc>,
    ) -> Result<(), BusinessVerificationError> {
        if !self.status.can_transition_to(target) {
            return Err(BusinessVerificationError::InvalidTransition {
                from: self.status,
                to: target,
            });
        }
    ...
    ) -> Result<Option<String>, BusinessVerificationError> {
        let trimmed = note.map(|n| n.trim().to_owned());
        match (trimmed, required_for) {
            (None, Some(action)) => Err(BusinessVerificationError::EmptyReviewerNote { action }),
            (Some(s), Some(action)) if s.is_empty() => {
                Err(BusinessVerificationError::EmptyReviewerNote { action })
            }
            (Some(s), _) => {
                let len = s.chars().count();
                if len > MAX_REVIEWER_NOTE_LEN {
                    return Err(BusinessVerificationError::ReviewerNoteTooLong { actual: len });
                }
                Ok(Some(s))
            }
            (None, None) => Ok(None),
    ...
    /// # Errors
    ///
    /// 현재 상태가 `Pending` 이 아니면 [`BusinessVerificationError::InvalidTransition`].
    /// `note` 가 2000자 초과 시 [`BusinessVerificationError::ReviewerNoteTooLong`].
    pub fn approve(
        &mut self,
        ...
        self.transition_to(BusinessVerificationStatus::Approved, at)?;
    ...
    ///
    /// 현재 상태가 `Pending` 이 아니면 [`BusinessVerificationError::InvalidTransition`].
    /// `note` 가 비어있으면 [`BusinessVerificationError::EmptyReviewerNote`].
    /// `note` 가 2000자 초과 시 [`BusinessVerificationError::ReviewerNoteTooLong`].
    pub fn reject(
        &mut self,
        reviewer: Id<UserMarker>,
        ...
        self.transition_to(BusinessVerificationStatus::Rejected, at)?;
        self.reviewer_id = Some(reviewer);
        self.reviewer_note = normalized;
        self.reviewed_at = Some(at);
        Ok(())
    ...
    pub fn request_more_info(
        ...
        self.transition_to(BusinessVerificationStatus::NeedsMoreInfo, at)?;
        self.reviewer_id = Some(reviewer);
        self.reviewer_note = normalized;
        self.reviewed_at = Some(at);
        Ok(())
    ...
    /// # Errors
    ///
    /// 현재 상태가 `NeedsMoreInfo` 가 아니면 [`BusinessVerificationError::InvalidTransition`].
    pub fn resubmit(
        &mut self,
        submitted_documents: serde_json::Value,
        at: DateTime<Utc>,
    ) -> Result<(), BusinessVerificationError> {
        self.transition_to(BusinessVerificationStatus::Pending, at)?;
