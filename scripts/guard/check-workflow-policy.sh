    ...
    END {
      ...
      for (i=1; i<=job_count; i++) {
       ...
       if (required_count != 1) {
        ...
        if (step_count[required_job] != 1 || run_count[required_job] != 1 || exact_terminal_run[required_job] != 1) {
          fail("terminal job must have one canonical result-check step")
        }
        ...
        if (step_key_count[required_job SUBSEP "name"] != 1 \
          ...
          fail("terminal step must contain exactly name, env, and run keys")
        }
