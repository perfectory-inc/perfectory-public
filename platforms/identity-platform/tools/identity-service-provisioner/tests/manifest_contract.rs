...
const COMPOSE_SMOKE_BINDINGS: &str = include_str!("fixtures/compose-smoke-bindings.v1.json");
...
#[test]
fn compose_smoke_bindings_match_compiled_policy() -> Result<(), Box<dyn std::error::Error>> {
    let policy = parse_policy(POLICY)?;
    let bindings = parse_bindings(COMPOSE_SMOKE_BINDINGS)?;
    let resolved = resolve_manifest(policy, bindings)?;
