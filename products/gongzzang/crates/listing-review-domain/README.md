# listing-review-domain

`ListingReview` (Listing Review Queue) 도메인 (RDS 동적) crate에요.
...
- `ListingReviewRepository` trait — `find_by_id` / `find_pending` / `find_by_listing` / `save` (OCC).
...
- Business Verification Queue 와 달리 Listing Review Queue 는 `status` enum 없이 *Optional `ListingReviewDecision`* 으로 pending/decided 구분.

## 의존

- `shared-kernel` (`Id`, `UserMarker`, `ListingMarker`, `ListingReviewMarker`).
