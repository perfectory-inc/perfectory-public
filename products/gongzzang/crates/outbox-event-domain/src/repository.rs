...
//! Publisher 워커 (sub-project 4) 는 [`claim_unpublished`] 로 polling, 외부 발행 성공
//! 후 [`mark_published`] 호출.
//!
//! [`save`]: OutboxRepository::save
//! [`claim_unpublished`]: OutboxRepository::claim_unpublished
...
pub trait OutboxRepository: Send + Sync {
    ...
    /// 미발행 이벤트를 원자적으로 claim — publisher 워커 사용.
    ///
    /// claim 된 행은 `worker_id` 소유의 lease 를 가지며, `lease_for` 후 만료돼요.
    ///
    /// # Errors
    ///
    /// DB 통신 실패 시 [`RepoError::Database`].
