...
import { getFoundationPlatformEventInboxRecord } from "@/lib/foundation-platform/event-inbox";
...
const webhookSecret = "test-foundation-platform-webhook-secret-32-valid";
process.env.FOUNDATION_PLATFORM_WEBHOOK_SECRET = webhookSecret;

const { POST } = await import("@/app/foundation-platform/events/route");
  ...
  return {
    ...
    payload: {
      ...
      artifact_manifest_url:
        "https://foundation-platform.example.com/artifacts/anchor-snapshot.json",
...
function makeRequest(body: unknown, headers: Record<string, string> = {}) {
  ...
  return new NextRequest("http://localhost:3000/foundation-platform/events", {
    method: "POST",
    headers: {
      "content-type": "application/json",
      "x-foundation-platform-event-id": eventId,
      "x-foundation-platform-event-type": eventType,
      "x-foundation-platform-outbox-scope": scope,
      "x-foundation-platform-signature": signBody(timestamp, bodyText),
...
function makeUnsignedRequest(body: unknown, headers: Record<string, string> = {}) {
  ...
  request.headers.delete("x-foundation-platform-signature");
  return request;
}
...
describe("POST /foundation-platform/events", () => {
  beforeEach(async () => {
    vi.clearAllMocks();
    vi.unstubAllGlobals();
  ...
  it("accepts a foundation-platform gold pointer event and invalidates catalog cache", async () => {
    ...
    expect(mockRevalidateTag).toHaveBeenCalledWith("foundation-platform-catalog", { expire: 0 });
  });

  it("rejects object paths in Gold pointer version metadata", async () => {
    ...
    await expect(res.json()).resolves.toEqual({ reason: "invalid_event", status: "rejected" });
  });

  it("acknowledges duplicate foundation-platform events without applying side effects twice", async () => {
  ...
  it("acknowledges duplicate foundation-platform event bursts without repeated side effects", async () => {
    const first = await POST(makeRequest(eventBody()));
  ...
  it("rejects events whose required header values do not match the body", async () => {
    const res = await POST(
      makeRequest(eventBody(), {
        "x-foundation-platform-event-id": "0196f0b0-3e01-7000-8000-000000000999",
      }),
    );
    const json = await res.json();
  ...
  it("rejects foundation-platform events without a valid webhook signature", async () => {
  ...
  it("rejects foundation-platform events with stale webhook signatures", async () => {
    const body = eventBody();
    const bodyText = JSON.stringify(body);
    const staleTimestamp = (Math.floor(Date.now() / 1000) - 600).toString();
    const res = await POST(
      new NextRequest("http://localhost:3000/foundation-platform/events", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-foundation-platform-event-id": eventId,
          "x-foundation-platform-event-type": eventType,
          "x-foundation-platform-outbox-scope": scope,
          "x-foundation-platform-signature": signBody(staleTimestamp, bodyText),
          "x-foundation-platform-timestamp": staleTimestamp,
        },
        body: bodyText,
      }),
    );
  ...
  it("accepts a foundation-platform parcel anchor snapshot event without invalidating listing page cache", async () => {
    vi.stubGlobal(
      "fetch",
    ...
    const res = await POST(
      makeRequest(anchorEventBody(), {
        "x-foundation-platform-event-type": anchorEventType,
      }),
    );
    const json = await res.json();
  ...
  it("returns retryable failure when Rust API inbox write fails for anchor events", async () => {
    ...
    const json = await res.json();
    const inbox = await getFoundationPlatformEventInboxRecord(eventId);
  ...
  it("rejects parcel anchor snapshot events with non-HTTPS artifact manifest URLs", async () => {
    const res = await POST(
      makeRequest(
        anchorEventBody(
          {},
          { artifact_manifest_url: "http://foundation-platform.example.com/anchors.json" },
        ),
        {
          "x-foundation-platform-event-type": anchorEventType,
        },
  ...
  it("rejects unsupported foundation-platform event types", async () => {
    ...
    const res = await POST(
      makeRequest(
        ...
        {
          "x-foundation-platform-event-type": unsupportedType,
        },
      ),
    );
    const json = await res.json();
