# ADR 0052: 정적 아카이브는 자기가 대체하는 소스에서 굽는 조건을 읽는다
...
**개정 각주 (2026-08-24).** 결정은 그대로다. CI 가 이 결정을 실행한 증명에서 결함 하나를 찾아,
1항의 "동적 소스에서 읽는다"에 **읽는 시점**의 조건이 붙는다는 것이 드러났다.

Martin 은 `auto_bounds: calc` 로 소스의 extent 를 **기동 시점에 한 번** 계산하고 다시 계산하지
않는다. 계약에 고정된 Martin 실행 파일에서 세 상태를 재서 확인했다.

```
빈 표에 대해 기동                 -> TileJSON 에 `bounds` 키가 아예 없다
행을 넣고, 같은 프로세스 유지      -> 여전히 `bounds` 키가 없다
프로세스를 재시작, 표에 행 있음    -> "bounds":[127.0,36.0,127.1,36.1]
```

`scripts/tiles/boundary-slice-proof.sh` 는 승격 전후 대비를 증명해야 하므로 Martin 을 `complex`
승격 **전에** 띄운다. 그래서 그 Martin 은 빈 view 에 대해 계산한 답을 광고하고 있었고, 굽는
조건을 거기서 읽으려 하면 실패한다. 가운데 줄이 더 중요하다: 일부 행이 있을 때 기동한 Martin 은
이후 적재가 행을 늘려도 **옛 extent 를 계속 광고한다.** 그것을 그대로 믿고 구우면 bbox 밖의
산업단지가 조용히 빠진 아카이브가 나온다.

따라서 1항에 두 조건을 더한다.

1. **광고를 읽기 전에 소스가 현재 데이터에 대해 답하게 만든다.** 증명은 승격 뒤 Martin 을
   재시작한 다음 TileJSON 을 읽는다. 배포된 Martin 이 이미 발행된 projection 위에 올라오는
   것과 같은 상태다.
2. **광고한 extent 가 서빙하는 행을 덮는지 확인하고, 아니면 굽지 않는다.**
   `ST_Covers(ST_Expand(ST_MakeEnvelope(<광고 bbox>, 4326), 1e-9), ST_Extent(geom))` 를 PostGIS
   에서 판정한다. 없는 bbox 는 여기서 멈추고, 낡은 bbox 는 `martin-cp` 에 닿지 못한다.

읽는 곳은 바뀌지 않았다. 여전히 동적 소스의 TileJSON 이고, zoom 범위는 여전히 승격된 release 의
`catalog.vector_tile_release_layer` 와 대조한다.
