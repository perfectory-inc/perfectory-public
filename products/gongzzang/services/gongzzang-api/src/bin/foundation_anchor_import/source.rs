...
use gongzzang_persistence::foundation_anchor::find_inbox_event_payload;
use sqlx::PgPool;

use super::config::{ImportSource, ImporterConfig};
use super::error::AnchorImporterError;
use super::foundation_anchor_import::parse_anchor_manifest;
...
fn load_local_import_source(
    ...
    foundation_platform_updated_at: DateTime<Utc>,
) -> Result<LoadedImportSource, AnchorImporterError> {
    let manifest_bytes =
...
async fn load_event_payload_import_source(
    ...
    Ok(LoadedImportSource {
        ...
        foundation_platform_updated_at: event_config.foundation_platform_updated_at,
    })
}
...
pub fn event_artifact_config_from_payload(
    ...
    let foundation_platform_updated_at =
        parse_rfc3339_utc(&payload_string(payload, "published_at")?)?;
...
impl ArtifactHttpClient {
    ...
    async fn fetch_bytes(&self, url: &reqwest::Url) -> Result<Vec<u8>, AnchorImporterError> {
        execute(
            &self.breaker,
            &self.policy,
            "foundation_platform.anchor_artifact.fetch",
            || {
