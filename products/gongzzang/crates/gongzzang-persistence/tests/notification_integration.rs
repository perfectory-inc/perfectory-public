...
use chrono::Utc;
use gongzzang_persistence::notification::PgNotificationRepository;
...
async fn mark_read_moves_from_unread_to_read() {
    ...
    repo.mark_read(&n.id, &user_id, read_at, ctx)
        .await
        .expect("mark_read");
...
async fn mark_read_idempotent_on_already_read() {
    ...
    repo.mark_read(&n.id, &user_id, Utc::now(), test_ctx())
        .await
...
#[tokio::test]
async fn mark_read_does_not_change_another_users_notification() {
    let pool = setup_test_pool().await;
    truncate_all(&pool).await;
    let owner = seed_user(&pool, "zsub-nt-owner", "nt-owner@example.com").await;
    let other_user = seed_user(&pool, "zsub-nt-other", "nt-other@example.com").await;
    let repo = PgNotificationRepository::new(pool.clone());

    let notification = make_notification(owner.clone(), NotificationKind::ListingApproved);
    repo.insert(&notification, test_ctx()).await.unwrap();

    repo.mark_read(&notification.id, &other_user, Utc::now(), test_ctx())
        .await
        .expect("cross-user mark_read remains idempotent");

    assert_eq!(repo.find_unread_by_user(&owner).await.unwrap().len(), 1);
}
