...
6. **시드는 고정 리터럴 적재 id를 쓴다.** publisher처럼 새 적재를 만들 수 없으므로, 미러 geometry가
   ...
   > **해소 (2026-08-05):** 완화를 **카디널리티에서 내용으로** 옮겼다. 카운트는 지오메트리 하나가
   > 다른 것으로 바뀐 편집에는 그대로 일치했고, 그것이 바로 저장된 행이 낡았는데 카운트로는 말할 수
   > 없는 경우다. 양쪽 다 `geometry_checksum_sha256`을 갖고 있으므로 이제 `pnu`로 정렬해 묶은
   > 다이제스트를 비교한다 — 카운트 검사를 포함하면서 "같은 수"와 "같은 행"을 구분한다.
   >
   > **시드가 새 정체성을 만들 수 없는 것은 결함이 아니다.** 릴리스는 불변이고 적재를 지명하므로,
   > 지오메트리가 바뀌면 새 적재와 **그것을 지명할 새 릴리스**가 함께 나와야 한다. 픽스처가 적재 id만
   > 몰래 바꾸는 것은 이 원장이 막으려는 바로 그 형태다. 실패 메시지도 둘 다 요구하도록 고쳤다.
   >
   > 이 항목의 "완전한 답은 아니다"는 정확했지만 이유가 달랐다. 남은 것은 픽스처가 정체성을 못
   > 만든다는 점이 아니라, 검사가 **행 수만 보고 있었다**는 점이었다.
