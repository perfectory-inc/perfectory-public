...
use super::{
    publish, publish_from_listing, BuildingServingManifest, ExportArtifactInput,
    ExportSummaryInput, ListingExpectation, ManifestPublishConfig, ManifestPublishInput,
};
...
fn temporary_root(label: &str) -> PathBuf {
    std::env::temp_dir().join(format!(
        "foundation-platform-building-serving-manifest-{label}-{}",
        uuid::Uuid::now_v7()
    ))
}
...
async fn seed_generation(
    ...
    for pnu in pnus {
        ...
        let key = building_by_pnu_serving_object_key(generation, pnu)?;
        store
...
fn summary(generation: u64, artifacts: Vec<ExportArtifactInput>) -> ExportSummaryInput {
    ExportSummaryInput {
        schema_version: "foundation-platform.building_by_pnu_serving_export_summary.v1".to_owned(),
...
async fn publishes_after_read_back_and_pins_the_generation() -> anyhow::Result<()> {
    ...
    let stored = store
        ...
        .await?;
    let parsed: BuildingServingManifest = serde_json::from_slice(&stored)?;

    std::fs::remove_dir_all(&root)?;
    assert_eq!(manifest.current_generation, 1);
    assert_eq!(parsed.current_generation, 1);
    assert_eq!(parsed.unit, "building-by-pnu");
...
async fn refuses_to_point_at_objects_that_are_not_there() -> anyhow::Result<()> {
    ...
    artifacts.push(ExportArtifactInput {
        pnu: PNU_B.to_owned(),
        object_key: building_by_pnu_serving_object_key(1, PNU_B)?,
...
async fn refuses_an_empty_bake_and_a_key_from_another_generation() -> anyhow::Result<()> {
    ...
    mismatched[0].object_key = building_by_pnu_serving_object_key(2, PNU_A)?;
...
async fn seed_documents(
    ...
    for pnu in pnus {
        let body = format!(
            "{{\"schema_version\":\"{BUILDING_DOCUMENT_SCHEMA_VERSION}\",\"pnu\":\"{pnu}\",\
...
async fn a_listing_that_disagrees_with_the_stated_count_is_refused() -> anyhow::Result<()> {
    ...
    std::fs::remove_dir_all(&root)?;
    let error = missing_shard
        .err()
        .ok_or_else(|| anyhow::anyhow!("a short listing was published"))?;
...
async fn a_document_from_another_snapshot_is_refused() -> anyhow::Result<()> {
    ...
    let error = stale
        .err()
        .ok_or_else(|| anyhow::anyhow!("a document of another gold snapshot was published"))?;
    assert!(
