...
|---|---|
| 작성일 | 2026-05-07 |
| 상태 | **Superseded by [ADR 0021](./0021-static-vector-tile-decomposition.md)** (2026-05-07 EOD) — A2+SW spike 결과 worker uncontrolled wall; 본 ADR 의 "전수 검토" 가 X9 (PMTiles 분해 → flat .pbf) / X10 (rawData transfer) 두 path 누락 — ADR 0021 이 X9 채택 (trick 0, mapbox-gl 표준 100%) |
...
| 폐기 | T3b.2 의 `/api/tiles/[...path]/route.ts` BFF proxy 실험 |

## 후속 정정 (2026-05-07 EOD)

본 ADR 의 채택 (A2-plugin + D-transport via Service Worker) 은 spike 검증 결과 **wall**:
...
T3b.x spike. 작동은 함 (parcelsFill: 71). 거부:
