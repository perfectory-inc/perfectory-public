...
crates/gongzzang-persistence → crates/{*-domain, shared-kernel, api-types} (ports만)
```
...
비용은 공개 문서의 고정 사용자 수나 출시 예산으로 결정하지 않는다. 환경별 트래픽,
저장량, 외부 API 호출량, 데이터베이스·compute profile을 입력으로 산정하고 실제 사용량과
청구 데이터로 갱신한다. 승인 예산과 현재 지출은 비공개 운영 증거로 관리한다.

| 비용 동인 | 산정 입력 |
|-----------|-----------|
| 트래픽 | 활성 세션, 요청/세션, peak factor |
| 저장소 | 저장 bytes, read/write 요청, egress, lifecycle |
| Compute/DB | instance profile, 실행 시간, 가용성 수준 |
| 외부 API | billable calls, 현재 단가, 포함 quota |

상세 산정식과 운영 가드: → [docs/cost/](./docs/cost/README.md)
