...
const VALID_ENTRY: &str = r#"{"schema_version":"foundation-platform.parcel_marker_anchor_artifact_entry.v1","pnu":"9999900101100090000","anchor_lng":127.12347023450,"anchor_lat":36.1234565,"anchor_srid":"EPSG:4326","algorithm":"polylabel","algorithm_version":"postgis-st_maximuminscribedcircle-v1","source_geometry_checksum_sha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}"#;

#[test]
fn parses_anchor_entry_into_import_row() {
    let entry = parse_anchor_entry(VALID_ENTRY).expect("entry");

    assert_eq!(entry.pnu, "9999900101100090000");
    assert!((entry.anchor_lng - 127.123_470_234_50).abs() < f64::EPSILON);
    assert!((entry.anchor_lat - 36.123_456_5).abs() < f64::EPSILON);
...
fn rejects_invalid_pnu_shape() {
    let err = parse_anchor_entry(&VALID_ENTRY.replace("9999900101100090000", "bad-pnu"))
...
fn parses_anchor_jsonl_rows_and_rejects_row_count_mismatch() {
    ...
    assert_eq!(rows[0].pnu, "9999900101100090000");
