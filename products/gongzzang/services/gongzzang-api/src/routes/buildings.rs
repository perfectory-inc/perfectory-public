...
//! Canonical catalog building data is read through Foundation Platform.
...
///
/// Implementations:
/// - production: `services/gongzzang-api/src/building_reader.rs::FoundationPlatformBuildingRegisterReader`
...
pub struct BuildingRegisterRecord {
    ...
    /// 옥탑 공용부 allocated area (㎡) reconciled from 전유공용면적. `None` when the
...
/// - `502 buildings-lookup-failed` when Foundation Platform lookup fails.
