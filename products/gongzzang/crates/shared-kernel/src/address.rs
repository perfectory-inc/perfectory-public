...
/// 도로명 주소 (예: `SYNTHETIC-CITY SYNTHETIC-ROAD 1`).
...
/// 지번 주소 (예: `SYNTHETIC-CITY SYNTHETIC-LOT 1-1`).
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(transparent)]
...
mod tests {
    ...
    #[test]
    fn road_typical() {
        let r = RoadAddress::try_new("SYNTHETIC-CITY SYNTHETIC-ROAD 1").expect("valid");
        assert_eq!(r.as_str(), "SYNTHETIC-CITY SYNTHETIC-ROAD 1");
    }

    #[test]
    fn road_trims() {
        let r = RoadAddress::try_new("  SYNTHETIC-CITY SYNTHETIC-ROAD 1  ").expect("trim");
        assert_eq!(r.as_str(), "SYNTHETIC-CITY SYNTHETIC-ROAD 1");
    }
    ...
    #[test]
    fn jibun_typical() {
        let j = JibunAddress::try_new("SYNTHETIC-CITY SYNTHETIC-LOT 1-1").expect("valid");
        assert_eq!(j.as_str(), "SYNTHETIC-CITY SYNTHETIC-LOT 1-1");
    }

    #[test]
    fn jibun_trims() {
        let j = JibunAddress::try_new("  SYNTHETIC-CITY SYNTHETIC-LOT 1-1  ").expect("trim");
        assert_eq!(j.as_str(), "SYNTHETIC-CITY SYNTHETIC-LOT 1-1");
    }
    ...
    fn types_are_distinct() {
        let r = RoadAddress::try_new("SYNTHETIC-CITY SYNTHETIC-ROAD 1").expect("ok");
        let j = JibunAddress::try_new("SYNTHETIC-CITY SYNTHETIC-LOT 1-1").expect("ok");
    ...
    fn display_round_trips() {
        let r = RoadAddress::try_new("SYNTHETIC-CITY SYNTHETIC-ROAD 1").expect("ok");
        assert_eq!(format!("{r}"), "SYNTHETIC-CITY SYNTHETIC-ROAD 1");

        let j = JibunAddress::try_new("SYNTHETIC-CITY SYNTHETIC-LOT 1-1").expect("ok");
        assert_eq!(format!("{j}"), "SYNTHETIC-CITY SYNTHETIC-LOT 1-1");
    }
    ...
    fn from_str_round_trips() {
        use std::str::FromStr;
        assert_eq!(
            RoadAddress::from_str("SYNTHETIC-CITY SYNTHETIC-ROAD 1")
                .unwrap()
                .as_str(),
            "SYNTHETIC-CITY SYNTHETIC-ROAD 1"
        );
        assert_eq!(
            JibunAddress::from_str("SYNTHETIC-CITY SYNTHETIC-LOT 1-1")
                .unwrap()
                .as_str(),
            "SYNTHETIC-CITY SYNTHETIC-LOT 1-1"
        );
    }
}
