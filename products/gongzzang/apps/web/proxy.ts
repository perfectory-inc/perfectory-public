...
// SP9 ADR 0036 / foundation-platform ADR 0004 채택 후 — 우리 origin 의 PMTiles/SW prefix
// 모두 폐기. 클라가 foundation-platform manifest 를 읽고, 표준 mapbox-gl `type:"vector"`
...
const PUBLIC_PATHS = [
  ...
  ROUTES.forbidden,
  AUTH_PATH_PREFIX,
  API.foundationPlatform.events,
  "/fonts",
];
const LISTING_MARKER_MASK_PREFIX = API.proxy.listingMarkerMasksPrefix;
const DEV_ONLY_PUBLIC_PATHS = ["/dev-tiles", "/dev-x9-test", "/preview-floors"];
...
function buildCspHeader(hostname: string, nonce: string): string {
  ...
  // ADR 0036 / foundation-platform ADR 0004: Gongzzang은 manifest consumer only.
