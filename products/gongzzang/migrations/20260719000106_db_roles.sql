...
begin
    execute format(
        'grant connect on database %I to gongzzang_app_writer',
        current_database()
    );
end $$;
...
begin
    execute format(
        'grant connect on database %I to gongzzang_app_reader',
        current_database()
    );
end $$;
...
begin
    execute format(
        'grant connect on database %I to gongzzang_audit_archiver',
        current_database()
    );
end $$;
...
-- 실제 사용자 (Pulumi에서 생성)
-- gongzzang_api_user (gongzzang_app_writer 역할 부여) — gongzzang-api 가 사용
