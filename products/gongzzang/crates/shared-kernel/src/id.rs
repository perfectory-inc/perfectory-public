...
/// `BusinessVerification` aggregate ID marker.
///
/// The persisted identifier keeps the compact `bvq_...` prefix defined by the
/// shared three-character ID wire format. Public Rust names remain explicit.
/// 사업자 인증 큐 — 4-status workflow (`pending`/`approved`/`rejected`/
/// `needs_more_info`) + version OCC + 24h SLA.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct BusinessVerificationMarker;
impl IdPrefix for BusinessVerificationMarker {
    const PREFIX: &'static str = "bvq";
}

/// `ListingReview` aggregate ID marker.
///
/// The persisted identifier keeps the compact `lrq_...` prefix defined by the
/// shared three-character ID wire format. Public Rust names remain explicit.
/// 매물 검토 큐 — decision-based workflow (`approve`/`reject`/`request_changes`,
/// `None` = pending) + version OCC + 12h SLA + `auto_check` (룰 기반).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ListingReviewMarker;
impl IdPrefix for ListingReviewMarker {
...
mod tests {
    ...
    #[test]
    fn new_business_verification_id_has_compact_prefix() {
        let id: Id<BusinessVerificationMarker> = Id::new();
        assert_eq!(id.as_str().len(), 30);
        assert!(id.as_str().starts_with("bvq_"));
    }

    #[test]
    fn new_listing_review_id_has_compact_prefix() {
        let id: Id<ListingReviewMarker> = Id::new();
        assert_eq!(id.as_str().len(), 30);
