...
선언이 애초에 갈라지지 않는다. 그러나 **도구는 ADR-0010에서 이미 기각했고 그 근거는 그대로
유효하다**: 26만 줄·1인 운영·호스트에 cargo 없음(모든 검증이 Docker 경유)·의도적으로 분리한
워크스페이스 4개, 그리고 DFINITY(Rust 60만 줄) 사례에서 rust-analyzer가 전체 코드베이스에서
깨지고 Cargo 파일을 이중 유지해야 했던 비용.
