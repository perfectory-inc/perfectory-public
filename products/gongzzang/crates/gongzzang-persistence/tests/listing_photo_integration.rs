...
async fn delete_removes_photo() {
    ...
    repo.delete(&photo.id, &listing_id, test_ctx())
        .await
        .expect("delete ok");
    let photos = repo.find_by_listing(&listing_id).await.unwrap();
    assert_eq!(photos.len(), 0);
...
async fn delete_nonexistent_returns_not_found() {
    ...
    let listing_id = seed_listing(&pool, "zsub-photo-missing", "missing@example.com").await;
    let err = repo.delete(&id, &listing_id, test_ctx()).await.unwrap_err();
    assert!(matches!(err, RepoError::NotFound));
}

#[tokio::test]
async fn delete_rejects_photo_from_a_different_listing() {
    let pool = setup_test_pool().await;
    truncate_all(&pool).await;
    let owner_listing_id = seed_listing(&pool, "zsub-photo-owner", "owner@example.com").await;
    let other_listing_id = seed_listing(&pool, "zsub-photo-other", "other@example.com").await;
    let repo = PgListingPhotoRepository::new(pool);

    let photo = make_photo(owner_listing_id.clone(), 0);
    repo.save(&photo, test_ctx()).await.unwrap();

    let err = repo
        .delete(&photo.id, &other_listing_id, test_ctx())
        .await
        .unwrap_err();
    assert!(matches!(err, RepoError::NotFound));
    assert_eq!(
        repo.find_by_listing(&owner_listing_id).await.unwrap().len(),
        1
    );
}
...
async fn delete_photo_audit_logs_with_action_delete() {
    ...
    repo.delete(&photo.id, &listing_id, delete_ctx)
        .await
        .expect("delete");
