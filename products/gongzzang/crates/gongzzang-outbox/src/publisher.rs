...
//!
//! 1 tick = `claim_unpublished` → 각 이벤트마다 `sink.publish` → 성공 시
...
/// publisher 호출 실패.
///
/// `tick` 자체가 실패하는 경우는 `claim_unpublished` 가 실패할 때예요. 개별 event
/// 의 sink 실패는 `tick` 의 `Ok(report)` 안에서 `failed` 카운터로 보고.
#[derive(Debug, Error)]
pub enum PublisherError {
    /// 저장소 호출 실패 — `claim_unpublished` 또는 catastrophic mark 실패.
...
///
/// `claim_unpublished(limit, worker_id, lease_for)` 로 가져온 이벤트들을 sequential 하게 sink 로 발행 +
/// 발행 성공 시 `mark_published`. 부분 실패는 `report.failed` 로만 누적 (다음 tick
/// 에 재시도).
///
/// # Errors
///
/// `claim_unpublished` 실패 시 [`PublisherError::Repo`] — 이 경우 batch 전체
...
pub async fn tick(
    ...
    let events = repo.claim_unpublished(limit, worker_id, lease_for).await?;
    ...
    for event in events {
        match sink.publish(&event).await {
            Ok(()) => match repo.mark_published(&event.id, worker_id, Utc::now()).await {
