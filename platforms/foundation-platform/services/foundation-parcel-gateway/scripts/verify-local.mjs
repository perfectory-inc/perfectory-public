...
const port = Number.parseInt(process.env.FOUNDATION_PARCEL_GATEWAY_LOCAL_PORT ?? "18788", 10);
const prefix = join(tmpdir(), "foundation-parcel-gateway-");
const proofRoot = await mkdtemp(prefix);
const persistDir = join(proofRoot, "r2-state");
const parcelFixture = join(proofRoot, "parcel.json");
...
try {
  const parcelBody = Buffer.from(`{"pnu":"${pnu}","generation":${servedGeneration}}\n`);
  ...
  if (!canonical.body.equals(parcelBody)) {
    throw new Error(
  ...
  process.stdout.write("OK foundation-parcel-gateway local Wrangler/R2 proof\n");
} catch (error) {
  ...
  const safePrefix = `${resolve(tmpdir())}${sep}foundation-parcel-gateway-`;
