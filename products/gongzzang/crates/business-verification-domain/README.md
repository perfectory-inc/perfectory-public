# business-verification-domain

`BusinessVerification` (Business Verification Queue) 도메인 (RDS 동적) crate에요.
...
- `BusinessVerificationRepository` trait — `find_by_id` / `find_pending` / `find_by_user` / `save` (OCC).
...
- `shared-kernel` (`Id`, `UserMarker`, `BusinessVerificationMarker`, `BusinessNumber`).
