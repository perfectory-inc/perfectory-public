...
describe("usePanelStack", () => {
  ...
  it("push calls router.push with serialized url", () => {
    const { result } = renderHook(() => usePanelStack());
    act(() => {
      result.current.push({ kind: "parcel", id: "9999900501107370000", view: "summary" });
    });
    expect(mockPush).toHaveBeenCalledWith("/listings?p=parcel%3A9999900501107370000.summary", {
      scroll: false,
    });
  });
  ...
  it("pop truncates the stack with router.replace", () => {
    mockSearchParams.set(
      "p",
      `parcel:9999900501107370000.summary>listing:${TEST_LISTING_ID}.summary`,
    );
    const { result } = renderHook(() => usePanelStack());
    act(() => {
      result.current.pop();
    ...
    expect(mockReplace).toHaveBeenCalledWith("/listings?p=parcel%3A9999900501107370000.summary", {
      scroll: false,
    });
  });
  ...
  it("refuses push when depth would exceed PANEL_DEPTH_MAX", () => {
    ...
    const long = Array.from({ length: 8 }, () => "parcel:9999900501107370000.summary").join(">");
