...
async function registerDevParcelLayer(
  ...
  setStatus("Fetching foundation-platform manifest...");
  const manifest = await fetchVectorTileManifest();
  if (isCancelled()) return;

  const artifact = getVectorTileArtifact(manifest, "parcels");
  if (!artifact) {
    setStatus("Foundation Platform manifest has no parcels artifact");
    return;
  }
  ...
  setStatus(`Foundation Platform parcels layer active: ${vectorSource.tiles[0]}`);
}
...
async function initializeDevX9Map(
  ...
  const map = new naverNs.maps.Map(container, {
    center: new naverNs.maps.LatLng(36.123447, 127.12347023472),
    zoom: 17,
    minZoom: 14,
    maxZoom: 19,
    gl: true,
