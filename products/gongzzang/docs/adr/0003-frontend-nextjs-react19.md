...
- **2026-09-09** — Next.js 를 `16.2.11` 에서 `16.3.3` 으로, Vitest 를 `4.1.7` 에서 `4.1.11` 로
  ...
  핀의 정본은 위 각주와 같다.
- **2026-09-09 (2)** — Vite 를 `6.4.2` 에서 `6.4.3` 으로 올렸다. 사유는 6.4.3 이 닫는 권고
  2건(`server.fs.deny` 우회, launch-editor UNC 경로)이고, 열린 기준선 행 2개가 이 승급으로
  함께 닫혔다. 새 parcel 게이트웨이 서비스도 같은 판을 쓴다. 핀의 정본은 위 각주와 같다.
