...
| Search | PostgreSQL 전문검색(`simple`) + `lindera` `5.1` 한국어 형태소(mecab-ko-dic); Meilisearch ADR only | 인메모리·Postgres 공유 계약 테스트 | provider 미선정 | provider 미선정 | 비계 테넌트 한정. HTTP route 없음. 형태소 근거는 [Intelligence ADR-0003](../platforms/intelligence-platform/docs/adr/0003-korean-morphology-in-rust.md) |
