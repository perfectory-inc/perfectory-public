//! `ListingReview` 도메인 에러.

use thiserror::Error;

/// `ListingReview` Aggregate 검증/전이 에러.
#[derive(Debug, Error, PartialEq, Eq)]
pub enum ListingReviewError {
    /// 이미 결정이 내려진 큐에 다시 결정 시도.
    ///
    /// `decision` 이 `Some(_)` 인 큐는 모든 `decide_*` 호출이 거부돼요.
    #[error("listing_review already decided")]
    AlreadyDecided,
...
mod tests {
    ...
    fn already_decided_message() {
        let err = ListingReviewError::AlreadyDecided;
        assert_eq!(err.to_string(), "listing_review already decided");
    }

    #[test]
    fn empty_reviewer_note_message_for_reject() {
        let err = ListingReviewError::EmptyReviewerNote { action: "reject" };
        assert_eq!(err.to_string(), "reviewer_note cannot be empty for reject");
    }

    #[test]
    fn empty_reviewer_note_message_for_request_changes() {
        let err = ListingReviewError::EmptyReviewerNote {
    ...
    fn reviewer_note_too_long_message() {
        let err = ListingReviewError::ReviewerNoteTooLong { actual: 2001 };
        assert_eq!(
            err.to_string(),
    ...
    fn auto_check_score_out_of_range_message() {
        let err = ListingReviewError::AutoCheckScoreOutOfRange { actual: 101 };
