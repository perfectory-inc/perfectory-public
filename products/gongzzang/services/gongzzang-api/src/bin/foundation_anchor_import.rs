//! Import Foundation Platform anchor artifact files into Gongzzang read models.
...
use gongzzang_persistence::foundation_anchor::{
    ...
    mark_inbox_event_processed, mark_inbox_event_processing, FoundationPlatformAnchorImport,
    FoundationPlatformAnchorImportReport,
};
use sqlx::{postgres::PgPoolOptions, PgPool};

#[path = "foundation_anchor_import/config.rs"]
mod config;
#[path = "foundation_anchor_import/error.rs"]
mod error;
#[path = "foundation_anchor_import/lock.rs"]
mod lock;
#[path = "foundation_anchor_import/source.rs"]
mod source;
#[path = "foundation_anchor_import/util.rs"]
mod util;

#[path = "../foundation_anchor_import.rs"]
mod foundation_anchor_import;

use config::{required_env, ImportSource, ImporterConfig};
use error::AnchorImporterError;
use foundation_anchor_import::{parse_anchor_manifest, parse_anchor_rows};
...
const MANIFEST_PATH_ENV: &str = "FOUNDATION_PLATFORM_ANCHOR_MANIFEST_PATH";
const OBJECT_PATHS_ENV: &str = "FOUNDATION_PLATFORM_ANCHOR_OBJECT_PATHS";
const ANCHOR_SNAPSHOT_ID_ENV: &str = "FOUNDATION_PLATFORM_ANCHOR_SNAPSHOT_ID";
const SOURCE_GEOMETRY_VERSION_ENV: &str = "FOUNDATION_PLATFORM_SOURCE_GEOMETRY_VERSION";
const PUBLISHED_AT_ENV: &str = "FOUNDATION_PLATFORM_ANCHOR_PUBLISHED_AT";
const EVENT_ID_ENV: &str = "FOUNDATION_PLATFORM_EVENT_ID";
const BATCH_LIMIT_ENV: &str = "FOUNDATION_PLATFORM_ANCHOR_IMPORT_BATCH_LIMIT";
    ...
    match run().await {
        Ok(report) => {
            tracing::info!(
                ...
                inserted_delta_count = report.inserted_delta_count,
                inserted_dirty_tile_count = report.inserted_dirty_tile_count,
                "foundation platform anchor import completed"
            );
            ExitCode::SUCCESS
        }
        Err(error) => {
            tracing::error!(error = %error, "foundation platform anchor import failed");
            ExitCode::FAILURE
        }
    }
}

async fn run() -> Result<FoundationPlatformAnchorImportReport, AnchorImporterError> {
...
async fn run_single_import(
...
) -> Result<FoundationPlatformAnchorImportReport, AnchorImporterError> {
    ...
    if let Some(lock) = event_lock {
        if let Err(release_error) = lock.release().await {
            ...
            tracing::error!(
                error = %release_error,
                "failed to release foundation platform event import advisory lock"
            );
        }
    }
    result
...
async fn run_pending_inbox_batch(
    ...
    let mut report = FoundationPlatformAnchorImportReport {
        upserted_anchor_count: 0,
        refreshed_listing_projection_count: 0,
        inserted_delta_count: 0,
        inserted_dirty_tile_count: 0,
    };
    ...
    for event_id in event_ids {
        ...
        match run_single_import(pool, &config).await {
            Ok(event_report) => {
                ...
                report.inserted_delta_count = report
                    .inserted_delta_count
                    .saturating_add(event_report.inserted_delta_count);
                report.inserted_dirty_tile_count = report
                    .inserted_dirty_tile_count
                    .saturating_add(event_report.inserted_dirty_tile_count);
            }
            Err(AnchorImporterError::InboxEventAlreadyLocked { .. }) => {
                tracing::info!(
                    event_id = %event_id,
                    "foundation platform anchor import event is already locked"
                );
            }
            Err(error) => {
                ...
                tracing::error!(
                    event_id = %event_id,
                    error = %error,
                    "foundation platform anchor import event failed in batch"
                );
            }
        }
    }
            ...
            if let Some(event_id) = &config.event_id {
                ...
                if let Err(mark_error) = mark_inbox_event_failed(pool, event_id, &reason).await {
                    tracing::error!(
                        event_id = %event_id,
                        error = %mark_error,
                        "failed to mark foundation platform anchor import event as failed"
                    );
                }
            }
            Err(error)
...
async fn run_import(
    ...
    let import = FoundationPlatformAnchorImport {
        ...
        foundation_platform_updated_at: source.foundation_platform_updated_at,
        rows,
    };
...
#[cfg(test)]
#[path = "foundation_anchor_import/tests.rs"]
mod tests;
