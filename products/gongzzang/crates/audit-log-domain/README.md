...
`AuditLog` 도메인 (RDS 동적) crate에요.

## 책임

- spec § 5.3 `audit_log` 테이블 매핑하는 Aggregate 정의해요.
- **불변 (immutable) — append-only**. `20260719000107_audit_immutable_trigger.sql`(구 V002) 트리거가 `UPDATE`/`DELETE`를 차단해요.
...
어떤 필드도 바꿀 수 없어요. 이건 audit immutable trigger 마이그레이션과 함께 컴파일 + 런타임
