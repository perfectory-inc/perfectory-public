...
//! byte-for-byte.
//!
//! Run with `cargo run -p gongzzang-api --bin generate-traffic-auth-policy`.
...
fn run() -> Result<(), String> {
    ...
    write_file(
        &root.join("services/gongzzang-api/src/traffic_auth_policy.rs"),
        &traffic_rs,
    )?;
    let marker_rs = render_listing_marker_policy_rs(&registry)?;
    write_file(
        &root.join("services/gongzzang-api/src/listing_marker_policy.rs"),
        &marker_rs,
    )?;
