...
6. **검증된 표준 도구 활용** — 타일 빌드는 tippecanoe와 PMTiles 공개 사양을 따른다.
...
| **자동강제 부재** | ETL smoke test (강남 PNU `9999900501107370000` Gold 에 존재 + Bronze sha256 검증) + GitHub Actions 실패 → Sentry 알림 |
...
zoom < 16 에서는 필지 안 그림 (data 양 폭증 방지). 검증된 프로토타입도 동일 LOD 정책.
...
```sql
SELECT * FROM listing
WHERE admin_code = '9999900501'  -- 역삼동
...
| **E. PMTiles 100% (Bronze + Gold) — 본 결정** | ✅ Phase 1-2 needs 충족, 비용 사실상 0, 검증된 프로토타입 패턴, 미래 확장 path 열림 |
...
- `crates/data-clients/r2-public-data/` PMTiles header parser 활성화 (historical; Foundation Platform-owned after ADR 0034)
...
- 현행 Foundation counterpart: [FP-ADR-0006](../../../../platforms/foundation-platform/docs/adr/0006-lakehouse-table-format-and-serving-architecture.md)
