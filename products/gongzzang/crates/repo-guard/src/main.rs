...
use guards::capability_layout::check_capability_layout;
...
fn run() -> Result<(), String> {
    ...
    match subcommand.as_str() {
        "capability-layout" => {
            ...
            let report = check_capability_layout(&repo_root)?;
            writeln!(
                io::stdout(),
                "capability-layout-ok business_crates={}",
                report.business_crates
            )
