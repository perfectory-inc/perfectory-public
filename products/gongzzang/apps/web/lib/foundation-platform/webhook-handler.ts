...
import {
  type FoundationPlatformEventInboxRecord,
  recordFoundationPlatformEventAccepted,
  recordFoundationPlatformEventDeadLetter,
  releaseFoundationPlatformEventReservation,
  reserveFoundationPlatformEvent,
} from "@/lib/foundation-platform/event-inbox";
...
// Foundation publishes an opaque artifact version, not an object-storage path.
// The producer contract does not require UUIDs, so keep this consumer compatible
// with any clean, path-free identifier.
const ArtifactVersionSchema = z
  .string()
  .min(1)
  .max(128)
  .regex(/^[A-Za-z0-9][A-Za-z0-9._:-]*$/);
const FOUNDATION_PLATFORM_SCOPE = "catalog";
const CATALOG_CACHE_TAG = "foundation-platform-catalog";

// Canonical foundation-platform header names — surfaces resolve these to the actual
// header they read (identity or preferred→fallback).
const FOUNDATION_PLATFORM_SIGNATURE_HEADER = "x-foundation-platform-signature";
const FOUNDATION_PLATFORM_TIMESTAMP_HEADER = "x-foundation-platform-timestamp";
const FOUNDATION_PLATFORM_INTERNAL_EVENTS_PATH = "/internal/foundation-platform/events";
...
const FoundationPlatformEventEnvelopeSchema = z
  .object({
    event_id: z.string().uuid(),
    event_type: z.string().min(1),
    occurred_at: z.string().datetime({ offset: true }),
    scope: z.literal(FOUNDATION_PLATFORM_SCOPE),
  ...
  .object({
    ...
    payload: z
      .object({
        ...
        current_version: ArtifactVersionSchema,
...
type FoundationPlatformEventEnvelope = z.infer<typeof FoundationPlatformEventEnvelopeSchema>;
export type GoldPointerEvent = z.infer<typeof GoldPointerEventSchema>;
...
// ---------------------------------------------------------------------------
// Event handler registry (module-level to avoid per-request allocation)
// ---------------------------------------------------------------------------
...
/** Handle one signed Foundation Platform event. */
export async function handleWebhookPost(req: NextRequest): Promise<NextResponse> {
  ...
  const envelope = FoundationPlatformEventEnvelopeSchema.safeParse(body.value);
  ...
  const reservation = await reserveFoundationPlatformEvent({
    event_id: envelope.data.event_id,
    event_type: envelope.data.event_type,
    scope: envelope.data.scope,
  });
  if (reservation.status === "duplicate") {
    const response = duplicate(reservation.record);
    console.info({
      event_source: "foundation-platform",
      event_id: envelope.data.event_id,
      event_type: envelope.data.event_type,
      status: "duplicate",
    });
  ...
  if (accepted === "durable_inbox_unavailable") {
    await releaseFoundationPlatformEventReservation(envelope.data.event_id);
  ...
  await recordFoundationPlatformEventAccepted({
    event_id: envelope.data.event_id,
    event_type: envelope.data.event_type,
    scope: envelope.data.scope,
...
async function handleGoldPointerEvent(value: unknown): Promise<AcceptedResponse | undefined> {
  const event = parseGoldPointerEvent(value);
  if (!event) return undefined;

  invalidateCatalogCache();

  return acceptedResponse(event, "invalidate_catalog_cache");
}

/** Parse the published Gold pointer contract without invoking Redis or cache side effects. */
export function parseGoldPointerEvent(value: unknown): GoldPointerEvent | undefined {
...
async function handleParcelAnchorSnapshotEvent(
  ...
  const persisted = await persistFoundationPlatformEvent(parsed.data);
  if (!persisted) return "durable_inbox_unavailable";

  return acceptedResponse(parsed.data, "enqueue_anchor_projection_import");
}
...
async function persistFoundationPlatformEvent(
  event: FoundationPlatformEventEnvelope,
): Promise<boolean> {
  try {
    const res = await fetch(
      `${env.NEXT_PUBLIC_API_BASE_URL}${FOUNDATION_PLATFORM_INTERNAL_EVENTS_PATH}`,
      {
...
function isValidWebhookSignature(req: NextRequest, bodyText: string): boolean {
  const timestamp = requiredHeader(req, FOUNDATION_PLATFORM_TIMESTAMP_HEADER);
  const signature = requiredHeader(req, FOUNDATION_PLATFORM_SIGNATURE_HEADER);
  ...
  const expectedHex = createHmac("sha256", env.FOUNDATION_PLATFORM_WEBHOOK_SECRET)
...
function validateRequiredHeaders(
  ...
  const eventId = requiredHeader(req, "x-foundation-platform-event-id");
  const eventType = requiredHeader(req, "x-foundation-platform-event-type");
  const scope = requiredHeader(req, "x-foundation-platform-outbox-scope");
...
function duplicate(record: FoundationPlatformEventInboxRecord): DuplicateResponse {
  return {
...
async function deadLetter(event: FoundationPlatformEventEnvelope, reason: string) {
  await recordFoundationPlatformEventDeadLetter({
    event_id: event.event_id,
