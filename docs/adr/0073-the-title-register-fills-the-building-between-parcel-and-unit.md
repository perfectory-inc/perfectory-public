...
- **2026-09-03, 관 전체 가동 후 실측.** Silver 8,051,204동 → canonical 7,603,113동. 이 결정이
  요구한 채움율 기록과, 그 실측이 낳은 남은 부채 둘:

  ### 남은 부채 (이 ADR로 닫히지 않는 것)

  1. **orphan 410,705동(5.1%)** — PNU 가 `catalog.parcel` 에 없어 붙지 못하고 세어진 건물.
     호 투영(0.4%)의 12배로, 건물 PNU 에는 필지 경계 원천이 덮지 않는 대역(산번지 등)이
     섞인다는 뜻이다. 이 수는 관의 결함이 아니라 **필지 커버리지의 구멍 지도**이며, 시군구별
     분포 조사가 다음 작업이다. 적재 요약의 객체별 orphaned 카운트가 원자료다.
  2. **`built_year` NULL 1,241,528동(15.4%)** — 표제부 사용승인일이 비었거나 형식 밖.
     마이그레이션 20260903000003 이 이 실측의 결과다. 다른 원천(허가일 등)으로 보강 가능한지의
     조사는 별도 결정으로 남긴다 — 이 관은 지어내지 않는다.

  둘 다 `docs/roadmap/foundation-goals.md` 의 과제 목록이 소유한다.
