...
2026-08-05에 **여덟 ADR을 모두 실물과 대조했다.** 기록된 37항목 중 열린 것은 **19개**다. 나머지
18개는 이미 닫혔거나(8), 애초에 부채가 아니거나(정정·감사 결과·교훈 기록 3), 지켜야 할 코드가 아직
없어 도달할 수 없거나(4), 의도적 결정이다(3). 닫힌 8개 중 **셋은 아무도 ADR에 적지 않은 채로**
닫혀 있었고(그중 하나는 자기를 인용한 CI 주석까지 달려 있었다), 다섯은 이 대조를 하면서 닫았다.
...
| [0011 테스트 실행 집합 완전성](../adr/0011-test-execution-set-completeness.md) | 4 | 3 | 정적 가드, 자격증명 레인, 임포트 게이트 | 4 |
