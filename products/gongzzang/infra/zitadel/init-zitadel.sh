...
#
# Pre-requisites:
#   docker compose --env-file infra/zitadel/.env -f infra/zitadel/docker-compose.yml up -d
...
# Alpine 3.19 OCI index resolved with `docker buildx imagetools inspect` on 2026-07-23.
TOKEN=$(docker run --rm -v "${PAT_VOLUME}:/pat-out" \
...
Verify: open http://localhost:18443/ui/console with the admin credentials
injected through the untracked Zitadel env file.
================================================================
EOF
