...
use gongzzang_persistence::listing_photo::PgListingPhotoRepository;
...
use product_identity_infrastructure::middleware::{auth_layer, AuthState};
...
async fn async_main() -> Result<(), StartupError> {
    ...
    let jti_denylist: Option<Arc<dyn product_identity_infrastructure::jti_denylist::JtiDenylist>> =
        redis_pool_shared.as_ref().map(|pool| {
            let dl: Arc<dyn product_identity_infrastructure::jti_denylist::JtiDenylist> = Arc::new(
                product_identity_infrastructure::jti_denylist::RedisJtiDenylist::new(
                    pool.as_ref().clone(),
                ),
            );
            dl
    ...
    let notification_repo: Arc<dyn notification_domain::repository::NotificationRepository> =
        Arc::new(
            gongzzang_persistence::notification::PgNotificationRepository::new(
                auth_event_state.pool.clone(),
            ),
        );
    let listing_repo_shared: Arc<dyn listing_domain::repository::ListingRepository> = Arc::new(
        gongzzang_persistence::listing::PgListingRepository::new(auth_event_state.pool.clone()),
    );
    let bookmark_repo: Arc<dyn bookmark_domain::repository::BookmarkRepository> = Arc::new(
        gongzzang_persistence::bookmark::PgBookmarkRepository::new(auth_event_state.pool.clone()),
    );
    ...
    let foundation_events_state = routes::foundation_events::FoundationPlatformEventsState {
        pool: auth_event_state.pool.clone(),
        internal_auth_secret: auth_event_state.internal_auth_secret.clone(),
    };
    let foundation_events_router: Router<()> = Router::new()
        .route(
            "/internal/foundation-platform/events",
            axum::routing::post(routes::foundation_events::post_foundation_platform_event),
        )
        .with_state(foundation_events_state);
    ...
    let app = public
        ...
        .merge(foundation_events_router)
