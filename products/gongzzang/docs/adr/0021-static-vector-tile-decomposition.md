...
mb.addSource("parcels", {
  ...
  tiles: ["https://tiles.example.invalid/gold/v3/parcels/{z}/{x}/{y}.pbf"],
  minzoom: 14,
  maxzoom: 17,
  promoteId: "PNU",
});
...
- A2+SW (ADR 0019 채택) → spike 결과 *Service Worker 가 mapbox-gl worker thread fetch 미통제* — web platform spec 의 worker isolation. **wall**.
...
- 환경변수: `NEXT_PUBLIC_PMTILES_BASE_URL=...` → `NEXT_PUBLIC_TILES_BASE_URL=https://tiles.example.invalid/gold/v3/` 로 변경. 우리 manifest 가 정한 prefix.
