...
This runbook covers Gongzzang runtime calls to Foundation Platform and Foundation Platform
...
Foundation Platform catalog reads:
...
- Check Foundation Platform health, network egress, and recent deploys first.
...
Anchor snapshot events are durable in `foundation_platform_event_inbox`. The importer
uses `FOUNDATION_PLATFORM_EVENT_ID` to mark an event `processing`, import the
...
with the same `FOUNDATION_PLATFORM_EVENT_ID` or run the importer without local
...
Foundation Platform owns PNU anchor source data. Gongzzang owns listing semantics,
