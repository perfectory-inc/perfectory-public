...
1. **`r2_layout.rs`가 소문자 unit key를 요구한다.** `unit_key` 컬럼은 대소문자를 허용하므로
   ...
   > **해소 (2026-08-05) — 다만 이 항목의 서술이 틀렸다.** "publisher가 더 좁다"는 포함 관계가
   > 아니었다. 세 방향으로 어긋나 있었다: 컬럼은 대문자를 **허용**하고 `.`를 거부했으며, publisher는
   > 대문자를 거부하고 `.`를 **허용**했고 첫 글자에 아무 제약이 없었다. 어느 쪽도 다른 쪽의
   > 부분집합이 아니므로 **어떤 단위 키도 한쪽 규칙만 믿을 수 없었다.** 한 축만 보고 "더 좁다"고
   > 적은 것이 이 항목이 오래 열려 있던 이유다.
   >
   > 컬럼을 교집합 `^[a-z][a-z0-9_-]{0,127}$`로 좁혔다(`20260805000001`). 단위 키는 이름이 아니라
   > **주소**이므로 세 규칙에 각각 저장소 쪽 근거가 있다 — 객체 키는 대소문자를 구분하므로 `Parcels`와
   > `parcels`는 한 단위에 두 접두사가 되고, 파생 이름은 확장자가 뒤에 붙는 파일명 어간이라 `.`가
   > 어간을 모호하게 만들며(Martin이 정적 소스를 정확히 거기서 쪼개 발견한다), 숫자나 구분자로
   > 시작하는 키는 이름이 아니라 조각으로 읽힌다.
   >
   > Rust 쪽 정본은 `catalog_domain::is_publication_unit_key`이며 규칙이 존재하는 이유인 객체 키
   > 파생 함수 옆에 둔다. `r2_layout.rs`는 그것을 부른다 — 규칙을 그 자리에 다시 적은 것이 애초에
   > 어긋남이 생긴 경로다.
   >
   > SQL과 Rust는 정의를 나눠 쓸 수 없으므로 **비교한다.**
   > `a_publication_unit_key_is_spelled_the_same_way_in_both_languages`가 불일치 축마다 하나씩,
   > 길이 경계는 양쪽으로 후보를 넣어 두 판정이 같은지 본다. 통과만이 아니라 거부도 확인했다 —
   > Rust만 대문자를 허용하게 바꾸면 `"Parcels"`를 지목하며 실패한다.
2. **정적 승격의 fallback 쓰기 순서가 규약이다.** `fallback_distinct_check`가 활성과 같은
   ...
   > **정정 (2026-08-05):** 이 항목은 아직 **도달할 수 없는 부채**다. 규약을 지켜야 할 코드가
   > 없기 때문이다 — `promote_tile_layer_static`은 여전히 포트 기본 구현이 에러이고
   > ([ADR-0014](./0014-serving-generation-tracks-one-unit-source-selection.md) 남은 부채 1),
   > `fallback_release_id`를 쓰는 프로덕션 코드는 저장소에 없다(포트 정의와 테스트 두 곳뿐).
   > 정적 승격을 구현하는 변경이 이 순서를 함께 가져가야 하며, **그 변경 전에는 강제할 대상이
   > 없다.** 남은 일 목록에서 세지 말 것.
