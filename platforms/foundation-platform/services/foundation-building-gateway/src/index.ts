...
function parseManifestGeneration(raw: unknown): number | null {
  ...
  if (manifest.schema_version !== 1 || manifest.unit !== "building-by-pnu") return null;
...
async function fetchBuilding(
  ...
  const generation = await resolveGeneration(bucket, ctx).catch(() => null);
  if (generation === null) {
    return withCors(
  ...
  try {
    object = await bucket.get(key, { onlyIf: request.headers });
  } catch {
    return withCors(
      new Response(null, { status: 503, headers: { "Cache-Control": "no-store" } }),
      origin,
...
export { canonicalPnu, corsHeaders, fetchBuilding, parseAllowedOrigins, parseManifestGeneration };
