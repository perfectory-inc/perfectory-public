...
//! - `GET /readyz` — readiness (DB ping 성공? Redis 미설정이면 skip). 다운
//!   시 503 → load balancer 가 트래픽 cut, restart trigger X
//! - `GET /readyz/db` — DB 단독 debug. non-production router only.
//!
...
///
/// `/readyz/db` is intentionally opt-in so production can omit the debug DB-only
...
pub fn public_router(state: HealthState, expose_db_debug_route: bool) -> Router<()> {
    ...
    let router = if expose_db_debug_route {
        router.route("/readyz/db", get(db_health))
    } else {
        router
    };
...
/// `GET /readyz/db` — DB 단독 health check (debug / on-call).
///
/// **SECURITY**: production omits this route from `public_router`; `/readyz`
...
mod tests {
    ...
    async fn public_router_omits_db_debug_route_when_disabled(
        ...
        let response = app
            .oneshot(Request::builder().uri("/readyz/db").body(Body::empty())?)
            .await?;
