...
#[test]
fn wgs84_repository_synthetic_point() {
    // Repository-reserved synthetic WGS84 point.
    let p = PointSrid::try_new_wgs84(127.123_470_234_54, 36.123_456_6).expect("valid WGS84");
    assert!((p.lng - 127.123_470_234_54).abs() < f64::EPSILON);
    assert!((p.lat - 36.123_456_6).abs() < f64::EPSILON);
...
fn to_geo_point_maps_lng_to_x_lat_to_y() {
    let p = PointSrid::try_new_wgs84(127.123_470_234_54, 36.123_456_6).expect("valid");
    let geo = p.to_geo_point();
    assert!((geo.x() - 127.123_470_234_54).abs() < f64::EPSILON);
    assert!((geo.y() - 36.123_456_6).abs() < f64::EPSILON);
}
...
fn polygon_with_hole() {
    ...
    let hole = LineString(vec![
        Coord {
            x: 127.123_470_234_10,
            y: 36.123_440,
        },
        Coord {
            x: 127.123_470_234_20,
            y: 36.123_440,
        },
        Coord {
            x: 127.123_470_234_20,
