...
## 5. Foundation Platform Cache Invalidation

Foundation Platform events can invalidate catalog-related cache and trigger anchor projection import.
...
Static reference tile lifecycle is Foundation Platform-owned.

Gongzzang's map client may consume manifests and immutable tile URLs, but Gongzzang does not own the build/promote lifecycle for Foundation Platform reference tiles.

## 7. Guardrails

The traffic/auth, Foundation Platform event-receiver, and PNU-anchor PBF marker
contracts are enforced in CI and pre-commit hooks. The Foundation Platform boundary is
guarded by `scripts/lefthook/foundation-ownership-boundary.sh`; the traffic/auth policy
artifacts are regenerated from the registry with
`cargo run -p gongzzang-api --bin generate-traffic-auth-policy`.
