...
`gold/manifest.<previous_version>.json` retention and cleanup belongs to foundation-platform
Catalog. The Gongzzang workflow remains present only as a disabled audit stub, and
`etl-base-layer cleanup-manifest-backups` exits with the foundation-platform handover notice.
...
- manifest cleanup 후속 구현은 foundation-platform Catalog 로 이관
