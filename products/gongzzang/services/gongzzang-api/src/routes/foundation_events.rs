//! Internal receiver for Foundation Platform events forwarded by the Next.js public route.
...
use gongzzang_persistence::foundation_anchor::{
    self, FoundationPlatformEventInboxInsert, FoundationPlatformEventInboxStatus,
};
...
/// Route state.
#[derive(Clone)]
pub struct FoundationPlatformEventsState {
...
/// Foundation Platform event envelope forwarded by the public receiver.
#[derive(Debug, Deserialize)]
pub struct FoundationPlatformEventEnvelope {
    event_id: String,
    event_type: String,
    occurred_at: String,
    scope: String,
...
/// Handler for `/internal/foundation-platform/events`.
pub async fn post_foundation_platform_event(
    State(state): State<FoundationPlatformEventsState>,
    headers: HeaderMap,
    Json(event): Json<FoundationPlatformEventEnvelope>,
) -> impl IntoResponse {
    handle_event(state, headers, event).await
}
...
async fn handle_event(
    state: FoundationPlatformEventsState,
    headers: HeaderMap,
    ...
    match foundation_anchor::insert_inbox_event(&state.pool, &row).await {
        Ok(_) => {
            tracing::info!(
                event_id = %event.event_id,
                event_type = %event.event_type,
                effect = %effect,
                "foundation_platform_event_accepted"
            );
            json_response(
                StatusCode::ACCEPTED,
                serde_json::json!({
        ...
        Err(error) => {
            tracing::error!(
                ...
                event_type = %event.event_type,
                error = %error,
                "failed to persist foundation platform event"
            );
            json_response(
...
fn inbox_insert_from_event(
    event: &FoundationPlatformEventEnvelope,
) -> Result<FoundationPlatformEventInboxInsert, &'static str> {
    ...
    match event.event_type.as_str() {
        GOLD_POINTER_EVENT_TYPE => Ok(FoundationPlatformEventInboxInsert {
            event_id: event.event_id.clone(),
            event_type: event.event_type.clone(),
            scope: event.scope.clone(),
            effect: "invalidate_catalog_cache".to_owned(),
            status: FoundationPlatformEventInboxStatus::Accepted,
        ...
        PARCEL_ANCHOR_SNAPSHOT_EVENT_TYPE => {
            ...
            Ok(FoundationPlatformEventInboxInsert {
                event_id: event.event_id.clone(),
                event_type: event.event_type.clone(),
                scope: event.scope.clone(),
...
mod tests {
    ...
    fn anchor_snapshot_event_maps_to_pending_import_inbox_row() -> Result<(), &'static str> {
        let event = FoundationPlatformEventEnvelope {
            ...
            payload: serde_json::json!({
                ...
                "artifact_manifest_url": "https://foundation-platform.example.com/artifacts/anchor-snapshot.json",
    ...
    #[test]
    fn foundation_platform_event_maps_to_catalog_cache_invalidation() -> Result<(), &'static str> {
        let event = FoundationPlatformEventEnvelope {
            event_id: "0196f0b0-3e01-7000-8000-000000000003".to_owned(),
            event_type: GOLD_POINTER_EVENT_TYPE.to_owned(),
            occurred_at: "2026-07-02T00:00:00Z".to_owned(),
            scope: "catalog".to_owned(),
            payload: serde_json::json!({
                "type": GOLD_POINTER_EVENT_TYPE,
                "schema_version": 1,
                "complex_id": "018f0000-0000-7000-8000-000000000001",
                "current_version": "0196e7e0-3c20-7000-8000-100000000001",
                "source_snapshot_id": "bronze:molit:2026-07-02",
                "iceberg_snapshot_id": "111"
            }),
        };

        let row = inbox_insert_from_event(&event)?;
        assert_eq!(row.effect, "invalidate_catalog_cache");
        assert_eq!(row.event_id, event.event_id);
        Ok(())
    }
}
