...
pub trait ParcelInfoLookup: Send + Sync {
    ...
    /// # Errors
    ///
    /// - [`LookupError::Backend`] when the Foundation Platform API call fails.
    /// - [`LookupError::Parse`] when Foundation Platform returns an invalid contract payload.
...
pub enum LookupError {
    /// Foundation Platform API call failed or returned a non-success status.
    #[error("backend error: {0}")]
    Backend(String),
    /// Foundation Platform returned an invalid or inconsistent payload.
    #[error("parse error: {0}")]
    Parse(String),
}
