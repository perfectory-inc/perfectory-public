# ADR 0039 - Service-Owned Lakehouse With Foundation Platform Registry
...
data, court auction data, and market data. Foundation Platform owns Catalog/common data such as parcels,
...
Iceberg can govern queryable tables, but Foundation Platform's Lakehouse Registry remains the cross-service
...
The diagram defines the logical production topology and namespace contract; it is not an inventory
of currently provisioned resources. Account bindings, provisioning state, location hints, and storage
classes are private operational evidence and are not asserted by this ADR.

Gongzzang does not write Gongzzang-owned Onbid, court auction, listing, market, or media pipeline
objects into Foundation Platform-owned R2 namespaces.

Foundation Platform remains the top-level control plane. Its Lakehouse Registry records Gongzzang-owned
asset locations, active versions, lineage, quality evidence, and consumer bindings. Registry
metadata does not transfer data ownership from Gongzzang to Foundation Platform.
...
-> registers run/artifact/version in Foundation Platform Lakehouse Registry

Gongzzang app/API
-> does not guess canonical R2 keys
-> resolves active governed assets through Foundation Platform Registry/API contracts
```
...
- Writing Gongzzang-owned lakehouse data into a Foundation Platform-owned root `bronze/` or `gold/`
  namespace.
- Reintroducing V-World/data.go.kr Catalog ingestion crates into Gongzzang.
- Treating Foundation Platform R2 object keys as a public API.
...
Foundation Platform Catalog/raw-data storage remains outside Gongzzang runtime configuration. Gongzzang
resolves governed Catalog artifacts through Foundation Platform contracts instead of reading Foundation Platform
...
must treat that root-level namespace as Foundation Platform-owned current/legacy lakehouse material and not
...
- Foundation Platform remains the central control plane without becoming the owner of all business facts.
- R2 object layout, active version, lineage, and quality evidence are discoverable through one
  governed registry.
- Later AI/vector indexing can read registered assets across Foundation Platform and Gongzzang without
...
1. Do not delete existing R2 objects until Foundation Platform inventory classifies owner, lineage, and
   active/retention status.
2. Create or designate a Gongzzang-owned lakehouse namespace before new Onbid/court auction Bronze
   writes.
3. Add Foundation Platform Registry registration to Gongzzang pipeline completion.
4. Add CI checks that reject new shared root medallion prefixes without explicit owner namespace.
5. Keep Foundation Platform Catalog consumption through published API/event/artifact contracts.
