//! Integration tests for Foundation Platform anchor inbox and read-model import.
...
use gongzzang_persistence::foundation_anchor::{
    AnchorArtifactRow, FoundationPlatformAnchorImport, FoundationPlatformEventInboxInsert,
    FoundationPlatformEventInboxStatus,
};
use sqlx::{PgPool, Row};
...
static FOUNDATION_PLATFORM_ANCHOR_IMPORT_TEST_LOCK: OnceLock<Mutex<()>> = OnceLock::new();

async fn lock_foundation_anchor_import_tests() -> MutexGuard<'static, ()> {
    FOUNDATION_PLATFORM_ANCHOR_IMPORT_TEST_LOCK
...
async fn assert_anchor_reimport_behavior(
    pool: &PgPool,
    import: &FoundationPlatformAnchorImport,
    listing_id: &str,
) {
    let repeat_report = gongzzang_persistence::foundation_anchor::import_anchor_rows(pool, import)
        .await
        .unwrap();
    assert_eq!(repeat_report.inserted_delta_count, 0);
    assert_eq!(repeat_report.inserted_dirty_tile_count, 0);

    let mut changed_import = import.clone();
    changed_import.rows[0].source_geometry_checksum_sha256 = "c".repeat(64);
    let changed_report =
        gongzzang_persistence::foundation_anchor::import_anchor_rows(pool, &changed_import)
            .await
            .unwrap();
    assert_eq!(changed_report.inserted_delta_count, 1);
    assert_eq!(changed_report.inserted_dirty_tile_count, 0);
...
#[tokio::test]
async fn inbox_insert_is_idempotent_by_foundation_platform_event_id() {
    let _guard = lock_foundation_anchor_import_tests().await;
    let pool = setup_test_pool().await;
    truncate_all(&pool).await;

    let event = FoundationPlatformEventInboxInsert {
        ...
        status: FoundationPlatformEventInboxStatus::PendingImport,
    ...
    let first = gongzzang_persistence::foundation_anchor::insert_inbox_event(&pool, &event)
        .await
        .unwrap();
    let second = gongzzang_persistence::foundation_anchor::insert_inbox_event(&pool, &event)
        .await
        .unwrap();
    ...
    let count: i64 = sqlx::query_scalar("select count(*) from foundation_platform_event_inbox")
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(count, 1);
...
async fn inbox_event_payload_can_be_loaded_for_artifact_importer() {
    ...
    let event = FoundationPlatformEventInboxInsert {
        ...
        payload: serde_json::json!({
            ...
            "artifact_manifest_url": "https://foundation-platform.example.com/artifacts/manifest.json"
        }),
    ...
    gongzzang_persistence::foundation_anchor::insert_inbox_event(&pool, &event)
        .await
        .unwrap();

    let payload =
        gongzzang_persistence::foundation_anchor::find_inbox_event_payload(&pool, &event.event_id)
            .await
            .unwrap()
            .expect("payload");
    ...
    assert_eq!(
        ...
        "https://foundation-platform.example.com/artifacts/manifest.json"
    );
    assert!(
        gongzzang_persistence::foundation_anchor::find_inbox_event_payload(
            &pool,
...
async fn pending_anchor_import_event_ids_are_loaded_in_received_order_with_limit() {
    ...
    for (event_id, status) in [
        (
            ...
            FoundationPlatformEventInboxStatus::PendingImport,
        ),
        (
            "0196f0b0-3e01-7000-8000-000000000012",
            FoundationPlatformEventInboxStatus::Processing,
        ),
        (
            "0196f0b0-3e01-7000-8000-000000000013",
            FoundationPlatformEventInboxStatus::Processed,
        ),
        (
            "0196f0b0-3e01-7000-8000-000000000014",
            FoundationPlatformEventInboxStatus::Accepted,
        ),
    ] {
        let event = FoundationPlatformEventInboxInsert {
            event_id: event_id.to_owned(),
            event_type: if status == FoundationPlatformEventInboxStatus::Accepted {
                "catalog.industrial_complex.gold_pointer.published.v1".to_owned()
            } else {
                "catalog.parcel_marker_anchor.snapshot.published.v1".to_owned()
            },
            scope: "catalog".to_owned(),
            effect: if status == FoundationPlatformEventInboxStatus::Accepted {
            ...
            anchor_snapshot_id: if status == FoundationPlatformEventInboxStatus::Accepted {
                None
            } else {
                Some(format!("anchor-snapshot-{event_id}"))
            },
            source_geometry_version: if status == FoundationPlatformEventInboxStatus::Accepted {
                None
            } else {
    ...
    let ids =
        gongzzang_persistence::foundation_anchor::find_pending_anchor_import_event_ids(&pool, 1)
            .await
            .unwrap();
    ...
    let ids =
        gongzzang_persistence::foundation_anchor::find_pending_anchor_import_event_ids(&pool, 10)
            .await
            .unwrap();
...
async fn inbox_event_records_processing_processed_and_failed_states() {
    ...
    let processed_event = FoundationPlatformEventInboxInsert {
    ...
    gongzzang_persistence::foundation_anchor::insert_inbox_event(&pool, &processed_event)
        .await
        .unwrap();

    let claimed = gongzzang_persistence::foundation_anchor::mark_inbox_event_processing(
        &pool,
    ...
    gongzzang_persistence::foundation_anchor::mark_inbox_event_processed(
        &pool,
        &processed_event.event_id,
    )
    .await
    ...
    let failed_event = FoundationPlatformEventInboxInsert {
    ...
    gongzzang_persistence::foundation_anchor::insert_inbox_event(&pool, &failed_event)
        .await
        .unwrap();
    gongzzang_persistence::foundation_anchor::mark_inbox_event_processing(
        &pool,
        &failed_event.event_id,
    )
    .await
    .unwrap();
    gongzzang_persistence::foundation_anchor::mark_inbox_event_failed(
        &pool,
        &failed_event.event_id,
        "checksum mismatch",
    )
...
async fn processing_inbox_event_can_be_reclaimed_for_retry_after_worker_exit() {
    ...
    let first_claim = gongzzang_persistence::foundation_anchor::mark_inbox_event_processing(
        &pool,
        &event.event_id,
    )
    .await
    .unwrap();
    let retry_claim = gongzzang_persistence::foundation_anchor::mark_inbox_event_processing(
        &pool,
        &event.event_id,
    )
    .await
    ...
    gongzzang_persistence::foundation_anchor::mark_inbox_event_processed(&pool, &event.event_id)
        .await
        .unwrap();
...
async fn anchor_import_upserts_long_algorithm_version_and_refreshes_listing_projection() {
    ...
    let import = FoundationPlatformAnchorImport {
        ...
        foundation_platform_updated_at: published_at(),
    ...
    let report = gongzzang_persistence::foundation_anchor::import_anchor_rows(&pool, &import)
        .await
        .unwrap();

    assert_eq!(report.upserted_anchor_count, 1);
    assert_eq!(report.refreshed_listing_projection_count, 1);
    assert_eq!(report.inserted_delta_count, 1);
    assert_eq!(report.inserted_dirty_tile_count, 7);
    ...
    assert_anchor_reimport_behavior(&pool, &import, "lst_01HXY3NK0Z9F6S1B2C3D4E5F6G").await;
}
