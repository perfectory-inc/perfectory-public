...
fn derives_remote_artifact_config_from_event_payload() {
    let config = event_artifact_config_from_payload(&serde_json::json!({
        ...
        "artifact_manifest_url": "https://foundation-platform.example.com/artifacts/anchors/manifest.json",
    ...
    assert_eq!(
        ...
        "https://foundation-platform.example.com/artifacts/anchors/manifest.json"
    );
    assert_eq!(config.artifact_checksum_sha256, "a".repeat(64));
    assert_eq!(
        config.foundation_platform_updated_at,
...
fn resolves_object_url_relative_to_manifest_directory() {
    ...
    assert_eq!(
        url.as_str(),
        "https://foundation-platform.example.com/artifacts/anchors/gold/parcel-marker-anchors/shard-000001.jsonl"
    );
}
