...
// `Business Verification Queue` domain `RepoError` — has `Conflict` variant
impl MapFromSqlx for business_verification_domain::repository::RepoError {
    fn conflict() -> Self {
        Self::Conflict
    }
    fn database(msg: String) -> Self {
...
// `Listing Review Queue` domain `RepoError` — has `Conflict` variant
impl MapFromSqlx for listing_review_domain::repository::RepoError {
    fn conflict() -> Self {
        Self::Conflict
    }
    fn database(msg: String) -> Self {
...
// `FeaturedContent` domain `RepoError` — no `Conflict` variant
impl MapFromSqlx for featured_content_domain::repository::RepoError {
    fn conflict() -> Self {
        Self::Database("unexpected conflict in featured_content".to_owned())
    }
    fn database(msg: String) -> Self {
        Self::Database(msg)
    }
...
// `SystemAlert` domain `RepoError` — no `Conflict` variant
impl MapFromSqlx for system_alert_domain::repository::RepoError {
    fn conflict() -> Self {
        Self::Database("unexpected conflict in system_alert".to_owned())
    }
    fn database(msg: String) -> Self {
        Self::Database(msg)
    }
