//! `PgOutboxRepository` 통합 테스트 — `save` + `claim_unpublished` +
...
use std::time::Duration as LeaseDuration;
...
async fn save_persists_event() {
    ...
    let unpublished = repo
        .claim_unpublished(10, "test-worker", LeaseDuration::from_mins(1))
        .await
        .expect("claim");
...
#[tokio::test]
async fn claim_unpublished_excludes_published() {
    let pool = setup_test_pool().await;
    truncate_all(&pool).await;
...
async fn mark_published_updates_timestamp() {
    ...
    let event = make_outbox("test.created");
    repo.save(&event).await.expect("save");
    repo.claim_unpublished(1, "test-worker", LeaseDuration::from_mins(1))
        .await
        .expect("claim before mark");

    let now = Utc::now();
    repo.mark_published(&event.id, "test-worker", now)
        .await
        .expect("mark");

    // After publishing, claim_unpublished must not return it.
    let unpublished = repo
        .claim_unpublished(10, "test-worker", LeaseDuration::from_mins(1))
        .await
        .expect("claim");
...
async fn mark_published_nonexistent_returns_not_found() {
    ...
    let err = repo
        .mark_published(&id, "test-worker", Utc::now())
        .await
...
async fn mark_published_already_published_returns_not_found() {
    ...
    repo.claim_unpublished(1, "test-worker", LeaseDuration::from_mins(1))
        .await
        .expect("claim before mark");
    repo.mark_published(&event.id, "test-worker", Utc::now())
        .await
        .expect("first mark ok");

    let err = repo
        .mark_published(&event.id, "test-worker", Utc::now())
        .await
...
#[tokio::test]
async fn claim_unpublished_orders_by_occurred_at_asc() {
    let pool = setup_test_pool().await;
    truncate_all(&pool).await;
    let repo = PgOutboxRepository::new(pool);
