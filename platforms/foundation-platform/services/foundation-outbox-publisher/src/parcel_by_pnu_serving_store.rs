//! Where parcel by-PNU serving objects live, for every command that touches them.
//!
//! One definition shared by the command that bakes the objects
//! (`export-parcel-by-pnu-serving`) and the command that points the gateway at a generation
//! (`publish-parcel-by-pnu-serving-manifest`). The store a manifest is checked against and the
...
//! same bucket the gateway Worker binds (root ADR-0096). The lakehouse bucket also holds the
...
impl ParcelServingObjectStore {
    ...
    pub(crate) async fn write_object_create_only(
        ...
        sha256: &str,
            ...
            is_parcel_by_pnu_serving_object_key(key),
            "refusing to write {key} : it is not a canonical parcel by-PNU serving key"
        );
    ...
    pub(crate) async fn write_object_overwrite(
        ...
        sha256: &str,
            ...
            is_parcel_by_pnu_serving_object_key(key),
            "refusing to overwrite {key} : it is not a canonical parcel by-PNU serving key"
        );
        let request =
    ...
    pub(crate) async fn write_manifest(
        ...
        sha256: &str,
            ...
            is_parcel_by_pnu_serving_manifest_key(key),
            "refusing to write {key} : it is not the parcel by-PNU serving manifest key"
        );
        let policy = parcel_by_pnu_gateway_policy()?;
...
#[cfg(test)]
mod tests {
    use super::ParcelServingObjectStore;
    use crate::industrial_complex_gold_profile_store::ProfileStoreConfig;
    use std::path::PathBuf;

    fn temporary_root(label: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "foundation-platform-parcel-serving-store-{label}-{}",
            uuid::Uuid::now_v7()
        ))
    }

    const OBJECT_KEY: &str = "serving/parcels/by-pnu/v1/9999900000100000000.json";
    const MANIFEST_KEY: &str = "serving/parcels/by-pnu/manifest.json";
    ...
    async fn create_only_reuses_identical_bytes_and_refuses_different_bytes() -> anyhow::Result<()>
    {
        ...
        let store =
            ParcelServingObjectStore::open(&ProfileStoreConfig::Local { root: root.clone() })?;
