...
import { requireZitadelE2eCredentials } from "./zitadel-e2e-credentials";
...
test.describe("auth flow", () => {
  ...
  test("login → callback (Zitadel hosted UI) → profile", async ({ page }) => {
    ...
    const credentials = requireZitadelE2eCredentials(process.env);

    await page.goto("/login");
    await page.click('button[type="submit"]');
    // Zitadel hosted UI
    await page.waitForURL(/\/ui\/login\/login/);
    await page.fill('input[name="loginName"]', credentials.username);
    await page.click('button[type="submit"]');
