//! `BusinessVerificationRepository` port. **Optimistic locking** — `save` 는 `version` 컬럼으로
//! 동시 검토 충돌을 차단해요.
//!
...
use crate::entity::BusinessVerification;

/// `BusinessVerification` 저장/조회 포트.
#[async_trait]
pub trait BusinessVerificationRepository: Send + Sync {
    /// `id` 로 조회. 없으면 `Ok(None)`.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    #[allow(dead_code)]
    fn assert_obj_safe(_repo: &dyn BusinessVerificationRepository) {}
