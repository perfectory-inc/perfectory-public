//! `ListingReview` Aggregate (decision-based workflow + version OCC + 12h SLA + `auto_check`).
//!
...
use crate::decision::ListingReviewDecision;
use crate::errors::ListingReviewError;
...
pub struct ListingReview {
    /// 식별자 (`listing_review_<26 ULID>`).
    pub id: Id<ListingReviewMarker>,
    ...
    pub decision: Option<ListingReviewDecision>,
...
impl ListingReview {
    ...
    /// # Errors
    ///
    /// `auto_check_score` 가 `Some` 이고 100 초과 시 [`ListingReviewError::AutoCheckScoreOutOfRange`].
    ...
    ) -> Result<Self, ListingReviewError> {
        if let Some(score) = auto_check_score {
            if score > MAX_AUTO_CHECK_SCORE {
                return Err(ListingReviewError::AutoCheckScoreOutOfRange {
    ...
    ) -> Result<Option<String>, ListingReviewError> {
        let trimmed = note.map(|n| n.trim().to_owned());
        match (trimmed, required_for) {
            (None, Some(action)) => Err(ListingReviewError::EmptyReviewerNote { action }),
            (Some(s), Some(action)) if s.is_empty() => {
                Err(ListingReviewError::EmptyReviewerNote { action })
            }
            (Some(s), _) => {
                let len = s.chars().count();
                if len > MAX_REVIEWER_NOTE_LEN {
                    return Err(ListingReviewError::ReviewerNoteTooLong { actual: len });
                }
                Ok(Some(s))
            }
            (None, None) => Ok(None),
    ...
    /// 이미 결정된 경우 [`ListingReviewError::AlreadyDecided`] 로 거부.
    fn apply_decision(
        &mut self,
        ...
        note: Option<String>,
        at: DateTime<Utc>,
    ) -> Result<(), ListingReviewError> {
        if self.decision.is_some() {
            return Err(ListingReviewError::AlreadyDecided);
        }
    ...
    /// # Errors
    ///
    /// 이미 결정된 경우 [`ListingReviewError::AlreadyDecided`].
    /// `note` 가 2000자 초과 시 [`ListingReviewError::ReviewerNoteTooLong`].
    pub fn decide_approve(
        ...
        self.apply_decision(ListingReviewDecision::Approve, reviewer, normalized, at)
    }
    ...
    ///
    /// 이미 결정된 경우 [`ListingReviewError::AlreadyDecided`].
    /// `note` 가 비어있으면 [`ListingReviewError::EmptyReviewerNote`].
    /// `note` 가 2000자 초과 시 [`ListingReviewError::ReviewerNoteTooLong`].
    pub fn decide_reject(
        ...
        self.apply_decision(ListingReviewDecision::Reject, reviewer, normalized, at)
    }
