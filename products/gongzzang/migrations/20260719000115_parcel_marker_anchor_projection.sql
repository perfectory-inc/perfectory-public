-- Gongzzang-local projection of foundation-platform parcel marker anchors.
--
-- This table is not a product-owned coordinate source. It is a read model copied from
-- foundation-platform so Gongzzang can generate listing marker MVT/PBF tiles without calling
-- foundation-platform on every tile request.
...
create table parcel_marker_anchor (
    ...
    foundation_platform_updated_at timestamptz not null,
...
create index parcel_marker_anchor_snapshot_idx
    on parcel_marker_anchor(anchor_snapshot_id, foundation_platform_updated_at desc);
